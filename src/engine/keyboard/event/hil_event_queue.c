...
#include "hil_event_queue.h"
#include "revision.h"

#define __DEBUG_EVENT_QUEUE

/** \brief HIL event item */
typedef struct hil_event_chunk_struct
{
    hil_event_timed_t *space;
    size_t size;
} hil_event_chunk_t;
...
hil_event_timed_t event_space[EVENT_CHUNK_SIZE];
...
*/
extern size_t hil_event_queue_pop(hil_event_timed_t *const _value)
{
    ENTER();
    if ((queue.number_of_free_items == queue.chunk.size) && (queue.in == queue.out))
    {
...
extern size_t hil_event_queue_peek(const size_t _cursor, hil_event_timed_t *const _value)
{
    ENTER();
    if ((queue.number_of_free_items == queue.chunk.size) && (queue.in == queue.out))
    {
