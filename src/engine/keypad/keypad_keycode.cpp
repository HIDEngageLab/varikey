...
namespace engine
{
    namespace keypad
    {
        const uint8_t KeypadCode::key_code(const KEY_ID _identifier, const TABLE _mode)
        {
            auto get_value = [this](const code_table_t &_table, const uint8_t _index) -> int
            {
                ...
                if (_index < _table.size())
                {
                             ...
                             _index == (uint8_t)KEY_ID::KEY_74)
                    {
                        ...
                        {
                            up_key.cursor = (up_key.cursor < up_key.value.size()) ? up_key.cursor + 1 : 0;
                        }
                    }
                    else
                    {
            ...
            uint8_t value = 0xff;
            const uint8_t index = static_cast<const uint8_t>(_identifier);
            const TABLE mode = (_mode == TABLE::UNDEFINED) ? table : _mode;
            switch (mode)
            {
            case engine::keypad::TABLE::NUMBER:
                value = get_value(Keycode::number_value, index);
                break;
            case engine::keypad::TABLE::FUNCTIONAL:
                value = get_value(Keycode::functional_value, index);
                break;
            case engine::keypad::TABLE::NAVIGATION:
                value = get_value(Keycode::navigation_value, index);
                break;
            case engine::keypad::TABLE::TELEFON:
                value = get_value(Keycode::telephon_value, index);
                break;
            case engine::keypad::TABLE::MULTIMEDIA:
                value = get_value(Keycode::multimedia_value, index);
                break;
            case engine::keypad::TABLE::CUSTOM:
                value = get_value(Keycode::custom_value, index);
                break;
            default:
                break;
            }
