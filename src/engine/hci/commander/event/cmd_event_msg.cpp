...
extern void cmd_event_msg_req(chunk_t const *const _chunk, cmd_event_msg_t *const _msg)
{
    ...
    _msg->event.type = static_cast<hci_event_type_t>(_chunk->space[0]);
    if (_msg->event.type == HIL_EVENT_KEY)
    {
        _msg->event.common.key.id =    static_cast<hci_key_identifier_t>(_chunk->space[1]);
        _msg->event.common.key.level = static_cast<hci_key_level_t>(_chunk->space[2]);
    }
