...
namespace engine::backlight
{
    ...
    int const BLINK_ALERT = Settings::BLINK_ALERT_TIMEOUT;
    int const BLINK_MOUNT = Settings::BLINK_MOUNT_TIMEOUT;
    int const BLINK_SUSPEND = Settings::BLINK_SUSPEND_TIMEOUT;
    ...
    extern void perform(void)
    {
        ...
        case PROGRAM::ALERT:
            ...
            break;
        case PROGRAM::MOUNT:
            if (backlight.right.curent == Settings::MOUNTED_BACKLIGHT_RIGHT)
            {
        ...
        case PROGRAM::SUSPEND:
            ...
            break;
        case PROGRAM::CONST:
            if (!(backlight.left.next == backlight.left.curent &&
                  backlight.right.next == backlight.right.curent))
            {
                perform_step(10);
            }
            break;
    ...
    static void perform_step(const int _delay)
    {
        ...
        if (backlight.right.next != backlight.right.curent)
        {
            right_changed = true;
